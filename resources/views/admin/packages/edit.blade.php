...
@section('page_title', 'Ubah Paket WiFi')
    ...
    <form action="{{ route('admin.packages.update', $package) }}" method="POST" class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" id="speed_mbps" name="speed_mbps" value="{{ old('speed_mbps', $package->speed_mbps) }}" required min="1"
            ...
            <div>
                ...
                <input type="number" id="price" name="price" value="{{ old('price', (int)$package->price) }}" required min="0"
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="3"
                class="w-full border-gray-300 border rounded-lg px-3 py-2 text-sm focus:ring-purple-500 focus:border-purple-500">{{ old('description', $package->description) }}</textarea>
        </div>

        <div class="flex items-center">
            <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $package->is_active) ? 'checked' : '' }}
                class="h-4 w-4 text-purple-600 focus:ring-purple-500 border-gray-300 rounded">
            <label for="is_active" class="ml-2 block text-sm text-gray-900">Aktifkan paket</label>
        </div>
